...
export default function OwnerContracts() {
    ...
    const loadAccountAndContracts = async () => {
        try {
            if (address) {
                await fetchOwnerContracts(address);
                await fetchSignedContracts(address);
            } else {
    ...
    const fetchOwnerContracts = async (account) => {
        ...
        try {
            const contractsList = await contract.getContractsFromOwner(account);
            let signedOwnerContracts = [];
            let pendingOwnerContracts = [];
            let finalizedOwnerContracts = [];

            for (let contractId of contractsList) {
                const isFinished = await contract.isContractFinished(contractId);
                const isSigned = await contract.isContractSigned(contractId);
    ...
    const fetchSignedContracts = async (account) => {
        ...
        try {
            const contractsList = await contract.getContractsOfWorker(account);
